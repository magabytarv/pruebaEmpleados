...
class CI_DB_active_record extends CI_DB_driver {
	...
	function registrar_logs_pac($operacion)
	{		
	      if(isset($_SESSION['UID']) && isset($_GET['base']))
	      {
			    $result = $this->query("SELECT * FROM ".INFOSAC_DB.".usuario WHERE UID = {$_SESSION['UID']}");
			    $this->_reset_select();
	          $username = $result->first_row()->username;
	          $Path = matriz;
	          if (stristr($operacion, 'select') == false) 
	          {
	                  $subDir = '../../../';
	              $fp = false;
	              if ($Path != "") {
	                  $pathG   = $subDir . $Path . "/audit/";
	                  $archivo = $pathG . "AUD-" . date('Y-m-d') . "txt";
	                  $dir     = opendir($pathG);
	                  if ($dir == false)
	                      mkdir($pathG, 0777);
	                  $fp = fopen($archivo, 'a+');
	              ...
	              if ($fp) {
	                  $cabecera = "FECHA: " . date('Y-m-d H:i:s') . " El Usuario: " . $username . " ,uso la Base " . $_GET['base'] . " y ejecuto : ";
	                  $this->SaltoLinea($fp, 1);
	                  fwrite($fp, $cabecera);
	                  $this->SaltoLinea($fp, 1);
	                  fwrite($fp, $operacion);
	                  $this->SaltoLinea($fp, 1);
